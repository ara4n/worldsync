...
const drawDragLines = (d) => {
  selLines.push(createPolyline({ points: circle(sqX(d.c0), sqZ(d.r0), 0.52, 0.1), color: 0x86d3ff, width: 0.06 }))
  for (const [c, r] of d.targets) {
    selLines.push(createPolyline({ points: circle(sqX(c), sqZ(r), 0.3, 0.1), color: 0x7fe0a0, width: 0.05 }))
  }
}
...
const updateHover = (d) => {
  ...
  if (!hoverLine) hoverLine = createPolyline({ points: pts, color, width: 0.07 })
...
world.onupdate = (dt, time) => {
  ...
  if (checkedKing && !gameOver) {
    ...
    if (!checkRing) checkRing = createPolyline({ points: pts, color: 0xe06a6a, width: 0.07 })
  ...
  if (turnProp && !gameOver) {
    ...
    if (seat) {
      ...
      if (!seatRing) seatRing = createPolyline({ points: pts, color, width: 0.05 })
  ...
  if (gameOver && !winRing) {
    ...
    winRing = createPolyline({ points: circle(0, 0, 5.6, 0.15), color: col, width: 0.12 })
    if (!winShown) {
      winShown = true
